...
namespace pf
{
	...
	// Read entire file as raw bytes. Returns empty vector on failure.
	std::vector<uint8_t> read_file_bytes(const file_path& path);

	// Extract file paths from a drop_files message handle (Win32: HDROP from
	// WM_DROPFILES wParam). Calls DragFinish() before returning.
	std::vector<file_path> dropped_files(uintptr_t drop_handle);
