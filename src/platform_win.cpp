...
std::vector<uint8_t> pf::read_file_bytes(const file_path& path)
{
	std::ifstream f(utf8_to_utf16(path.view()).c_str(), std::ios::binary | std::ios::ate);
	if (!f) return {};
...
std::vector<pf::file_path> pf::dropped_files(const uintptr_t drop_handle)
{
	std::vector<file_path> result;
	const auto hdrop = reinterpret_cast<HDROP>(drop_handle);
	...
	for (UINT i = 0; i < count; i++)
	{
		...
		result.emplace_back(utf16_to_utf8(buf));
	}
	DragFinish(hdrop);
	return result;
}
